...
```
박현서:
3D 지형이 움직이는 것을 보면 항상 밤바다가 잔잔히 흐르는 것이 생각나 밤바다에 떠 있는 달과 튜브를 주제로 잡았습니다. 튜브는 Geometries 중 torus를 이용하여 만들고, 튜브의 색과 크기, 위치 조정, loadImage를 이용하여 튜브에 이미지 파일을 추가하는 등 수업 시간에 배웠던 내용들을 토대로 활용하여 결과물을 만들었다는 것에 매우 신기하고 뿌듯하였습니다. 그리고 Light와 Camera, Material을 사용하며 개념을 알고, 3D 지형 코드는 더 잘 이해하게 되었습니다. 프로젝트 과정 중 좌표가 아닌 시간에 따라 도형이 이동하게 만드는 것은 처음 시도해 보는 것이라 어렵게 느껴졌습니다. 하지만 팀원들 간 도움을 받으면서 time 변수를 지정하여 문제를 잘 해결할 수 있었습니다. 이번 팀 프로젝트에서 팀원들과 도움을 많이 주고받으면서 여럿이서 소통하고 협력하여 문제를 해결하는 것이 큰 힘이 되고 중요하다는 것을 깨닫게 되는 기회였습니다!
```
```
손정우:
```
